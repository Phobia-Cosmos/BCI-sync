...
def _load_cifar10_raw_image(dataset_path: str, img_name: str):
    parts = img_name.split("/")
    if len(parts) != 3:
        return None

    split, klass, file_name = parts
    ...
    if not os.path.isdir(os.path.join(root, "cifar-10-batches-py")):
        return None

    train = split == "train"
    cache_key = (root, train)
    if cache_key not in _CIFAR10_RAW_CACHE:
        from torchvision.datasets import CIFAR10

        dataset = CIFAR10(root=root, train=train, download=False)
        counts = {class_name: 0 for class_name in dataset.classes}
        index_by_name = {}
        for data_idx, target in enumerate(dataset.targets):
            class_name = dataset.classes[target]
            index_by_name[(class_name, f"{counts[class_name] + 1:04d}.png")] = data_idx
            counts[class_name] += 1
        _CIFAR10_RAW_CACHE[cache_key] = (dataset, index_by_name)

    dataset, index_by_name = _CIFAR10_RAW_CACHE[cache_key]
    data_idx = index_by_name.get((klass, file_name))
    if data_idx is None:
        return None
    return PIL.Image.fromarray(dataset.data[data_idx]).convert("RGB")
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(img_path):
            ...
            image = _load_cifar10_raw_image(self.dataset_path, img_name)
            if image is None:
                raise FileNotFoundError(
                    f"Image not found: {img_path}. If using CIFAR-10 raw data, "
                    "dataset_path must contain cifar-10-batches-py."
                )
        ...
        sample["true_label"] = int(true_label) if not pd.isna(true_label) and true_label != -1 else int(label)
