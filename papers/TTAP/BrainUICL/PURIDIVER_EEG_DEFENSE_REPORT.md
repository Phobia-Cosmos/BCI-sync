# PuriDivER-style 防护在 BrainUICL/EEG 上的实现与对比
...
**重要更正：本报告已有实验不是纯 PuriDivER-EEG。** 当前实现保留了 BrainUICL 的预训练 checkpoint、CPC teacher、teacher pseudo label、CEA、source replay 保护和 new/replay 权重，并额外定义了 teacher entropy 与 teacher/student JS divergence。原始 PuriDivER 假设数据流提供可能被污染的观测标签，只使用一个当前模型，并将 uncertainty 定义为 `1 - max softmax probability`。因此已有结果应解释为“BrainUICL + PuriDivER-style 防护”，不能用于判断只使用 PuriDivER 时的 EEG 持续学习性能。
