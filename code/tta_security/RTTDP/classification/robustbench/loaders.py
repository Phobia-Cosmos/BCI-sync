...
class CustomDatasetFolder(VisionDataset):
    ...
    def __init__(self, root, loader, extensions=None, transform=None, target_transform=None, is_valid_file=None):
        ...
        imagenet_list = os.environ.get(
            "RTTDP_IMAGENETC_LIST",
            "robustbench/data/imagenet_test_image_ids_10000.txt",
        )
        samples = make_custom_dataset(self.root, imagenet_list,
                                      'robustbench/data/imagenet_class_to_id_map.json')
        if len(samples) == 0:
    ...
    def __getitem__(self, index):
        ...
        # 
        if "poisoning" in domain:
            domain = 'poisoning_attack'
            path = path.replace("poisoning_", "")
...
class CustomModelNetDataset(data.Dataset):
    def __init__(self, samples, transform=None):
    ...
    def __getitem__(self, index):
        img, label, domain = self.samples[index]
        img = torch.tensor(img).permute(1, 0)
        return torch.tensor(img), torch.tensor(label).squeeze(), domain
