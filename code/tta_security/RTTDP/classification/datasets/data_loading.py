...
from datasets.corruptions_datasets import create_cifarc_dataset, create_imagenetc_dataset, create_modelnet40c_dataset
...
from datasets.attack_sampler import AttackBatchSampler, AttackSampleSampler, NonUniformAttackBatchSampler
...
def get_test_loader(setting: str, adaptation: str, dataset_name: str, preprocess: Union[transforms.Compose, None],
                    ...
                    rng_seed: int, delta_dirichlet: float = 0., batch_size: int = 128, shuffle: bool = False, workers: int = 4, poisoning_ratio: float = 0., attack_data_type: str = 'batch'):
    """
    ...
    if domain_name == "none":
        ...
        if dataset_name in ["cifar10_c", "cifar100_c"]:
            ...
            test_dataset = create_modelnet40c_dataset(dataset_name=dataset_name,
                                                      severity=severity,
                                                      data_dir=data_dir,
                                                      corruption=domain_name,
                                                      corruptions_seq=domain_names_all,
    ...
    if "poisoning_attack" in setting:
        if attack_data_type == 'batch' or attack_data_type == 'non_uniform_batch':
            if shuffle:
                sampler = torch.utils.data.RandomSampler(test_dataset)
            else:
                sampler = torch.utils.data.SequentialSampler(test_dataset)
            if attack_data_type == 'batch':
                batch_sampler = AttackBatchSampler(sampler, batch_size, drop_last=False, poisoning_ratio=poisoning_ratio)
            else:
                batch_sampler = NonUniformAttackBatchSampler(sampler, batch_size, drop_last=False, poisoning_ratio=poisoning_ratio)
            batch_size=1
            shuffle=False
        elif attack_data_type == 'sample':
            data_sampler = AttackSampleSampler(test_dataset, poisoning_ratio=poisoning_ratio)
        else:
            raise NotImplementedError('The ATTACK_DATA_TYPE of ' + attack_data_type + ' is not implemented.')
    return torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=workers, drop_last=False, batch_sampler=batch_sampler, sampler=data_sampler)
