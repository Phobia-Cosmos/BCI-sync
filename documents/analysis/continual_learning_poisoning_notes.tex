...
为什么选这个点？有三个原因：
\begin{itemize}
  \item \(w_s^\star\) 是干净任务 \(s\) 的最优点，所以
  \[
    \nabla_w\calL(w_s^\star,Z_s)=0.
  \]
  展开时常数项会消失，公式最简单。
  \item 我们想研究“投毒和正则化让模型偏离干净最优点多少”，所以自然以干净最优点为基准。
  \item \(Q_s\) 和 \(G_s\) 都是在干净数据上计算的局部几何量，表示没有攻击时任务本身的曲率和数据敏感性。
\end{itemize}
...
\[
  \Phi(w,Z)=\nabla_w\calL(w,Z).
\]
对两个变量 \(w\) 和 \(Z\) 做一阶 Taylor 展开：
\[
  \Phi(w_s,Z_s+\eta_s)
  \approx
  \Phi(w_s^\star,Z_s)
  +
  \frac{\partial \Phi}{\partial w}(w_s^\star,Z_s)(w_s-w_s^\star)
  +
  \frac{\partial \Phi}{\partial Z}(w_s^\star,Z_s)\vect(\eta_s).
\]
第一项为 0；第二项就是对 \(w\) 再求一次导，所以是 \(Q_s(w_s-w_s^\star)\)；第三项是“数据变化如何改变参数梯度”，所以记作 \(G_s\vect(\eta_s)\)。因此
\[
...
把它代回一阶最优性条件：
\[
  Q_s(w_s-w_s^\star)+G_s\vect(\eta_s)+H_s(w_s-w_{s-1})\approx0.
\]
接下来为什么引入 \(w_a^\star\)？因为 T2T 后面既要比较 \(w_t\) 和 \(w_{t-1}\)，又要比较 \(w_{t-1}\) 和 \(w_{t-2}\)。统一写成“相对某个参照点 \(w_a^\star\) 的偏移”，会让两次更新能放到同一种形式里。这个参照点不是新的假设，只是代数上选一个零点。选 \(a=t-1\) 可以看上一轮任务，选 \(a=t\) 可以把当前任务和上一任务的差异显式写出来。

具体整理方式是把
\[
  w_s-w_s^\star=(w_s-w_a^\star)-(w_s^\star-w_a^\star),
  \qquad
  w_s-w_{s-1}=(w_s-w_a^\star)-(w_{s-1}-w_a^\star)
\]
代入上一行，把含 \(w_s-w_a^\star\) 的项放左边，就得到
\[
  S_s(w_s-w_a^\star)
  \approx
...
\[
  ...
  -S_{t-1}^+G_{t-1}\vect(\eta_{t-1}).
\]
这里你可能会疑惑：上一轮更新里明明有正则化 \(H_{t-1}\)，为什么公式里看不到显式的 \(H_{t-1}\)？

原因是它没有消失，而是被吸收到
\[
  S_{t-1}=Q_{t-1}+H_{t-1}
\]
里面了。看代数最清楚。上一轮的一阶条件近似为
\[
  Q_{t-1}(w_{t-1}-w_{t-1}^\star)
  +G_{t-1}\vect(\eta_{t-1})
  +H_{t-1}(w_{t-1}-w_{t-2})
  \approx0.
\]
令上一轮更新量
\[
...
\[
  w_{t-1}-w_{t-1}^\star
  =
  u+(w_{t-2}-w_{t-1}^\star).
\]
代入：
\[
  Q_{t-1}\bigl[u+(w_{t-2}-w_{t-1}^\star)\bigr]
  +G_{t-1}\vect(\eta_{t-1})
  +H_{t-1}u
  \approx0.
\]
...
\[
  ...
  -S_{t-1}^+G_{t-1}\vect(\eta_{t-1}).
\]
因此 \(H_{t-1}\) 在 \(B_t=S_{t-1}^+Q_{t-1}\) 里通过 \(S_{t-1}^+\) 隐含存在，不是没了。
...
为什么 \(A_t\) 里显式出现 \(H_{t-1}\)，而 \(B_t\) 没有？因为二者描述的路径不同：
\begin{itemize}
  \item \(B_t\) 描述的是旧状态如何直接出现在上一轮更新 \(w_{t-1}-w_{t-2}\) 里。这里 \(H_{t-1}\) 和 \(Q_{t-1}\) 一起组成左边的总曲率 \(S_{t-1}\)，所以它藏在 \(S_{t-1}^+\) 里。
  \item \(A_t\) 描述的是旧状态如何先影响 \(w_{t-1}\)，再通过当前任务影响 \(w_t-w_{t-1}\)。旧状态传到 \(w_{t-1}\) 的那条“保留旧模型”的通道正是 \(H_{t-1}\)，所以它在路径中显式出现。
\end{itemize}
